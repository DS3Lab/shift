...
from .dataset import ClassificationTaskDataset, TaskDataset
...
class ClassificationTaskExpander(TaskExpander):
    ...
    def __call__(
        ...
        task_map,
        label_names=None,
        flatten_binary=False,
        force_multi_label=False,
        force_remove_multi_label=False,
        task_id=None,
        task_name=None,
        metadata=None,
        transform=None,
        ...
        for idx, (img, label) in enumerate(zip(task.images, task.labels)):
            if isinstance(label, list):
                ...
                if len(new_label) > 0:
                    ...
                    new_label = (
                        new_label[0]
                        if len(new_label) == 1 and is_single_label_task
                        else new_label
                    )
                    new_label = (
                        1 if flatten_binary and new_label == [0, 1] else new_label
                    )
        ...
        if force_remove_multi_label:
            multi_label_samples_mask = [
                isinstance(l, list) and len(l) != 1 for l in new_labels
            ]
            new_images = [
                img
                for idx, img in enumerate(new_images)
                if not multi_label_samples_mask[idx]
            ]
            new_labels = [
                l if isinstance(l, int) else l[0]
...
class MetaclassClassificationTaskExpander(TaskExpander):
    ...
    def __call__(
        ...
        assert all(
            all(x in task for x in ["task_id", "task_name", "label_map"])
            for task in task_map
        ), "task_map missing some required keys"
        ...
        for task_mapper in task_map:
            ...
            force_multi_label = (
                task_mapper["multi_label"] if "multi_label" in task_mapper else 0
            )
...
class BinaryClassificationTaskExpander(MetaclassClassificationTaskExpander):
    ...
    def __call__(self, task, labels=None, transform=None, target_transform=None):
        ...
        for label in labels:
            label_name = (
                task.label_names_map[label]
                if task.possible_label_names is not None
                else str(label)
            )
            ...
            binary_task["label_map"] = {
                l: 1 if l == label else 0 for l in task.possible_labels
            }
            binary_task["class_ids"] = [1, 0]
            binary_task["class_names"] = [label_name, "not {}".format(label_name)]
            task_map.append(binary_task)

        return super(BinaryClassificationTaskExpander, self).__call__(
            task,
            task_map,
            flatten_binary=True,
            transform=transform,
...
class UnionClassificationTaskExpander(TaskExpander):
    ...
    def __call__(
        ...
        if self._merge_duplicate_images:
            new_images, new_labels = self._merge_duplicate_images(
    ...
    def _disjoint_merge(self, tasks):
        ...
        for idx, t in enumerate(tasks):
            label_map[idx] = {
                l: label_counter + i for i, l in enumerate(t.possible_labels)
            }
            if t.possible_label_names is not None:
                label_names_map.update(
                    {label_counter + i: n for i, n in enumerate(t.possible_label_names)}
                )
            else:
                task_name = t.task_name if t.task_name is not None else str(idx)
                label_names_map.update(
                    {
                        label_counter + i: "{}_{}".format(task_name, l)
    ...
    def _merge_label_names(self, tasks, intersection=False):
        ...
        """
        if intersection:
            common_label_names = set.intersection(
                *[set(t.possible_label_names) for t in tasks]
            )
    ...
    def _remap_tasks(self, tasks, label_map):
        ...
        for idx, t in enumerate(tasks):
            ...
            new_images.extend(
                [
                    os.path.join(t.root, img) if isinstance(img, str) else img
                    for img in new_task.images
                ]
            )
...
class IntersectionClassificationTaskExpander(UnionClassificationTaskExpander):
    ...
    def __call__(
        ...
        if self._merge_mode == IntersectionClassificationTaskExpander.MERGE_LABELS:
            ...
            self._merge_mode == IntersectionClassificationTaskExpander.MERGE_LABEL_NAMES
        ):
            label_map, label_names_map = self._merge_label_names(
                tasks, intersection=True
            )
        ...
        *tasks : list (positional arguments)
        ...
        for idx, t in enumerate(tasks):
            new_task = remapper(
                ...
                target_transform=t.target_transform,
            )
