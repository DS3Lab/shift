...
def run_plot(tasks, repeat):
    ...
    for task in tasks:
        ...
        for i in tqdm(range(repeat)):
            run_result = search(config)
            if len(run_result) > 0:
                results.extend(run_result)
        df = pd.DataFrame(results)
        df.sort_values(by=['strategy'],key=lambda x: x.map(orders), inplace=True)
        create_line_plot(df, config['target']['name'], config['target']['title'])

@click.command()
@click.option('--task', default='task', help='Task Folder')
@click.option('--repeat', default=1, help='The number of times the search be repeated')
def run_plot_batch(task, repeat):
    logger.info(f"Processing {task}")
